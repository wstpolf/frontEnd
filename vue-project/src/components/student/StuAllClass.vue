...
export default {
  ...
  methods: {
    ...
    async fetchCourses() {
      try {
        const response = await axios.get(`http://localhost:8080/stu/allCourse/${this.$store.state.stuInform.id}`);
    ...
    async selectCourse(courseId) {
      try {
        const response = await axios.post('http://localhost:8080/stu/selectCourse', {courseId:courseId,studentId:this.$store.state.stuInform.id});
