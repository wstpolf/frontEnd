...
export default {
  ...
  methods: {
    ...
    async fetchCourses() {
      try {
        const response = await axios.get(`http://localhost:8080/stu/myClass/${this.$store.state.stuInform.id}`);
    ...
    async deleteCourse(courseId) {
      try {
        const response = await axios.delete(`http://localhost:8080/stu/notEnroll/${courseId}/${this.$store.state.stuInform.id}`);
        if (response.status === 200) {
