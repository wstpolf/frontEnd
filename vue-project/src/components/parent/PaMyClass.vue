    ...
    <el-main>
      ...
      <el-table :data="paginatedCourses" style="width: 100%;height: 80vh">
        ...
        <el-table-column label="操作">
          <template #default="{ row }">
            <el-button @click="deleteCourse(row.id)" type="danger" size="small">退课</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async fetchCourses() {
      try {
        const response = await axios.get(`http://localhost:8080/stu/myClass/${this.$store.state.childInform.id}`);
    ...
    // 编辑课程
    updateCourse(course) {
      this.editCourse = { ...course }; // 将课程数据赋值给 editCourse，填充到弹窗表单
      this.editCourseDialogVisible = true; // 显示编辑弹窗
    },
    // 提交编辑的课程
    async submitEditCourse() {
      try {
        const response = await axios.put(`http://localhost:8080/stu/updateCourse/${this.editCourse.id}`, this.editCourse);
    ...
    async deleteCourse(courseId) {
      try {
        const response = await axios.delete(`http://localhost:8080/stu/notEnroll/${courseId}/${this.$store.state.childInform.id}`);
        if (response.status === 200) {
